...
class CWMH(object):
    ...
    def sample(self, N, Nb):
        ...
        for s in range(Ns-1):
            ...
            samples[:, s+1], target_eval[s+1], acc[:, s+1] = self.single_update(samples[:, s], 
                                                                         target_eval[s])
            if (s % 5e2) == 0:
                print('Sample', s, '/', Ns)
...
class RWMH(object):
    ...
    def single_update(self, x_t, target_eval_t):
        # propose state
        x_star = x_t + self.scale*np.random.randn(self.n)

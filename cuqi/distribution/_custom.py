...
class UserDefinedDistribution(Distribution):
    ...
    .. code-block:: python
        ...
        dim1 = 1
        logpdf_func = lambda xx: -xp.log(std1*xp.sqrt(2*xp.pi))-0.5*((xx-mu1)/std1)**2
        sample_func = lambda : mu1 + std1*xp.random.randn(dim1,1)
...
class DistributionGallery(UserDefinedDistribution):
    ...
    def __init__(self, distribution_name,**kwargs):
        ...
        if distribution_name == "CalSom91":
            ...
            self.sig = 0.1
            self.delta = 1
            self.dfdx1 = lambda x1, x2: -(x1*(xp.sqrt(x2**2+x1**2)-1))/(self.sig**2*xp.sqrt(x2**2+x1**2))
            self.dfdx2 = lambda x1, x2: -(x2*(xp.sqrt(x2**2+x1**2)-1))/(self.sig**2*xp.sqrt(x2**2+x1**2))-(x2-1)/self.delta**2
            ...
            dim = 2
            mu = xp.zeros(dim)
            sigma = xp.diag(xp.linspace(0.5, 1, dim))
            R = xp.array([[1.0, 0.9 ],[0.9, 1.0]])
            ...
            self.m0, self.m1 = 0, 0
            self.s1 = 3
            self.f = lambda x, m, s: -0.5*xp.log(2*xp.pi) - xp.log(s) - 0.5*((x-m)/s)**2
            ...
            dim = 2
            self.m0, self.m1, self.m2 = xp.array([-1.5, -1.5]), xp.array([1.5, 1.5]), xp.array([-2, 2])
            ...
            # self.grad_logG = lambda x, mu, s: -xp.diag([1/s, 1/s])@(x-mu).T
            #
    ...
    def _CalSom91_logpdf_func(self, x):
        if len(x.shape) == 1:
            x = x.reshape((1, 2))
        return -1/(2*self.sig**2)*(xp.sqrt(x[:,0]**2+ x[:,1]**2) -1 )**2 -1/(2*self.delta**2)*(x[:,1]-1)**2

    def _CalSom91_grad_logpdf(self, x):
        if len(x.shape) == 1:
            x = x.reshape((1, 2))
        grad = xp.array([self.dfdx1(x[:, 0], x[:, 1]), self.dfdx2(x[:, 0], x[:, 1])])
        if x.shape[0] == 1:
            return grad.flatten()
        else:
            return grad
    ...
    def _funnel_grad_logpdf(self, x):
        if len(x.shape) == 1:
            x = x.reshape((1, 2))
        s0 = xp.exp(x[:, 1]/2)
        grad = xp.array([self.dfdx(x[:, 0], self.m0, s0), \
    ...
    def _mixture_logpdf_func(self, x):
        if len(x.shape) == 1:
            x = x.reshape((1, 2))
        return xp.log(self.G0.pdf(x) + self.G1.pdf(x) + self.G2.pdf(x))
    
    def _mixture_grad_func(self, x):
        if len(x.shape) == 1:
            x = x.reshape((1, 2))
        p1, p2, p3 = self.G0.pdf(x), self.G1.pdf(x), self.G2.pdf(x)
        scale = xp.nan_to_num(1 / (p1 + p2 + p3))
    ...
    def _squiggle_logpdf_func(self, x):
        if len(x.shape) == 1:
            x = x.reshape((1, 2))
        y = xp.zeros((x.shape[0], self.dim))
        y[:, 0], y[:, 1] = x[:, 0], x[:, 1] + xp.sin(5*x[:, 0])
    ...
    def _squiggle_grad_logpdf(self, x):
        ...
        gradx0, gradx1 = grad[0, :] + grad[1, :]*5*xp.cos(5*x[:, 0]), grad[1, :]
    ...
    def _donut_grad_logpdf(self, x):
        ...
        idx = xp.argwhere(r==0)
        r[idx] = 1e-16
        grad = xp.array([(x[:, 0]*((self.radius/r)-1)*2)/self.sigma2, \

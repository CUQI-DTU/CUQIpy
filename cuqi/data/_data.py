...
def satellite(size=None):
    ...
    stream = importlib.resources.files('cuqi.data').joinpath('satellite.mat').open('rb')
...
def astronaut(size=None, grayscale=True):
    ...
    """
    stream = importlib.resources.files('cuqi.data').joinpath('astronaut.npz').open('rb')
...
def camera(size=None):
    ...
    """
    stream = importlib.resources.files('cuqi.data').joinpath('camera.npz').open('rb')
    img = np.load(stream)['arr_0']
    if size:
        img = imresize(img, size) 
    return img
...
def cat(size=None, grayscale=True):
    ...
    """
    stream = importlib.resources.files('cuqi.data').joinpath('cat.npz').open('rb')
    img = np.load(stream)['arr_0']
    if grayscale:
        img = rgb2gray(img)
    if size:
...
def cookie(size=128, grayscale=True):
    ...
    stream = importlib.resources.files('cuqi.data').joinpath('cookie.png').open('rb')

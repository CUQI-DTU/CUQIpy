...
def grains(size=128, num_grains=34, seed=1):
    ...
    xG = xp.ceil(Nbig*rng.rand(num_grains, 1))
    yG = xp.ceil(Nbig*rng.rand(num_grains, 1))

    # set up voxel coordinates for distance computation
    xx   = xp.arange(1, Nbig+1)
    X, Y = xp.meshgrid(xx, xx, indexing='xy')
    X    = X.flatten(order='F')
    Y    = Y.flatten(order='F')

    # for centre pixel k [xG(k),yG(k),zG(k)] compute the distance to all the 
    # voxels in the box and store the distances in column k.
    distArray = xp.zeros((total_dim, num_grains))
    ...
    minIdx = xp.argmin(distArray, axis=1)
    ...
    img   = img[xp.ix_(dN+xp.arange(1, N+1)-1, dN+xp.arange(1, N+1)-1)]
    img   = img/img.max()
...
def shepp_logan(size=128):
    ...
    for i in range(nn):
        ...
        x   = Xn-x0
        y   = Yn-y0
        idd = ((x*xp.cos(phi) + y*xp.sin(phi))**2)/a2 + ((y*xp.cos(phi) - x*xp.sin(phi))**2)/b2
        idx = xp.where( idd <= 1 )
...
def threephases(size=128, p=70):
    ...
    # 1st
    xx     = xp.arange(1,N+1)-1
    I, J   = xp.meshgrid(xx,xx, indexing='xy')
    sigma1 = 0.025*N
    c1     = xp.random.rand(p,2)*N
    x1     = xp.zeros((N,N))
    for i in range(p):
        x1 += xp.exp(-abs(I-c1[i,0])**3/(2.5*sigma1)**3 - abs(J-c1[i,1])**3/sigma1**3)
    t1 = 0.35
    x1[x1 < t1]  = 0
    x1[x1 >= t1] = 2
    ...
    # 2nd
    sigma2 = 0.03*N
    c2     = xp.random.rand(p,2)*N
    x2     = xp.zeros((N,N))
    for i in range(p):
        x2 += xp.exp(-(I-c2[i,0])**2/(2*sigma2)**2 - (J-c2[i,1])**2/sigma2**2)
    t2 = 0.55
    x2[x2 < t2]  = 0
    x2[x2 >= t2] = 1
...
def p_power(size=128, relnz=0.3, p=2, seed=1): #relnz=0.65, p=2.3
    ...
    F = U*xp.exp(2*xp.pi*xp.sqrt(-1+0j)*P)
    F = abs(xp.fft.ifft2(F))
    f = -xp.sort(-F.flatten(order='F'))   # 'descend'
    k = round(relnz*N**2)-1
    F[F < f[k]] = 0
    x = F/f[0]
    if Nodd:
...
def rgb2gray(img):
    ...
    """
    return img @ xp.array([0.2125, 0.7154, 0.0721])
...
def imresize(image, size, **kwargs):
    ...
    """
    zoom_factor = size/xp.array(image.shape)

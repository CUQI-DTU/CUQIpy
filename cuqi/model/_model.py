...
from cuqi.geometry import Geometry, _DefaultGeometry1D, _DefaultGeometry2D,\
    _get_identity_geometries, ConcatenatedGeometries
import cuqi
...
class Model(object):
    ...
    def _2fun(self, geometry=None, is_par=True, **kwargs):
        ...
        if len(kwargs) >= 1 and isinstance(geometry,ConcatenatedGeometries):
        ...
        raise ValueError(
            ...
            +f"{ConcatenatedGeometries.__name__}.")
    ...
    def _2par(self, geometry=None, to_CUQIarray=False, is_par=False, **kwargs):    
        ...
        for i , (k, v) in enumerate(kwargs.items()):
            ...
            input_geom = geometry.geometries[i]\
                if isinstance(geometry, ConcatenatedGeometries) else geometry
    ...
    def is_stacked_args(self, *args, is_par=True):
        ...
        if not isinstance(self.domain_geometry, ConcatenatedGeometries):
            return False, args
    ...
    def _correct_distribution_dimension(self, distributions):
        ...
        if len(distributions) == 1:
            ...
            isinstance(self.domain_geometry, ConcatenatedGeometries):
            return all(
...
class LinearModel(Model):
    ...
    def adjoint(self, y, is_par=True):
        ...
        """
        return self._apply_func(func=self._adjoint_func,
                                fwd=False,
                                is_par=is_par,
                                **{'y':y})

...
class Geometry(ABC):
    ...
    def fun_shape(self):
        ...
        if not hasattr(self,'_fun_shape') or self._fun_shape is None:
            ...
            funvals = self.par2fun(xp.ones(self.par_dim))
    ...
    def funvec_shape(self):
        ...
        if not hasattr(self,'_funvec_shape') or self._funvec_shape is None:
            # Attempt to infer dimension
            funvecvals = self.fun2vec(self.par2fun(xp.ones(self.par_dim)))
    ...
    def variables(self, value):
        ...
        if isinstance(value,(int,xp.integer)):
        ...
        for key, value in vars(self).items():
            ...
            if isinstance(value, (tuple,list)) and isinstance(obj_value, (tuple,list)):
                for i in range(max(len(value),len(obj_value))):
                    if not xp.array_equiv(value[i],obj_value[i]):
                        return False
            # Else we check single element
            elif not xp.array_equiv(value, obj_value):
                return False 
        return True
...
class Continuous(Geometry, ABC):
    ...
    def _create_dimension(self, dim_grid):
        dim_grid_value_err_msg = "dim_grid should be int, tuple with one int element, list of numbers, 1D xp.ndarray, or None"
        if dim_grid is None:
            return None
        ...
        if isinstance(dim_grid,(int,xp.integer)):
            dim_grid = xp.arange(dim_grid)
        elif isinstance(dim_grid,(list,xp.ndarray)):
            dim_grid = xp.array(dim_grid)
...
class Continuous1D(Continuous):
    ...
    Parameters
    -----------
    grid : int, tuple, list or xp.ndarray
        1D array of node coordinates in a 1D grid (list or xp.ndarray), or number of nodes (int or tuple with one int element) in the grid. In the latter case, a default grid with unit spacing and coordinates 0,1,2,... will be created.
...
class Continuous2D(Continuous):
    ...
    def grid(self, value):
        if value is None: self._grid = None
            ...
            if len(value)!=2:
                raise NotImplementedError("grid must be a 2D tuple of int values or arrays (list, tuple or xp.ndarray) or combination of both")
...
class Discrete(Geometry):
    ...
    def _plot_envelope(self, lo_values, up_values, **kwargs):
        ...
        lo_values = xp.array(lo_values).flatten()
        up_values = xp.array(up_values).flatten()
        
        return plt.errorbar(self._ids, lo_values,
                            yerr=xp.vstack(
                                (xp.zeros(len(lo_values)), up_values-lo_values)),
                            **kwargs)

    def _plot_config(self):
        # Add at most 10 ticks including the first and last
        n_ticks = min(10, len(self.variables))
        tick_ids = xp.linspace(0, len(self.variables)-1, n_ticks, dtype=int)
...
#         self.mean = mean
#         self.std = std
#         XX, YY = xp.meshgrid(self.grid, self.grid, indexing='ij')
#         self.Sigma = cov_func(XX, YY)
#         self.L = xp.linalg.chol(self.Sigma)
...
class KLExpansion(Continuous1D):
    ...
    def coefs(self):
        ...
        if self._coefs is None or len(self._coefs) != self.num_modes:
            eigvals = xp.array(range(1, self.par_dim+1))  # KL eigvals
            self._coefs = xp.diag(1/xp.float_power(eigvals, self.decay_rate))
    ...
    def coefs_inverse(self):
        ...
        if self._coefs_inverse is None or\
            ...
            self._coefs_inverse = xp.diag(xp.float_power(xp.diag(self.coefs), -1))
...
class KLExpansion_Full(Continuous1D):
    ...
    def __init__(self, grid, std=1.0, cor_len=0.2, nu=3.0, axis_labels=None, **kwargs):
        ...
        modes = xp.arange(0,self.par_dim)

        self._coefs =  xp.float_power( tau2,gamma ) * xp.float_power(tau2+modes**2,-gamma)
...
class CustomKL(Continuous1D):
    ...
    def __init__(self, grid, mean=0, std=1.0, cov_func=None, trunc_term=None, axis_labels=None, **kwargs):
        ...
        if cov_func is None:
            ...
            cov_func = lambda x,y: 1.0 if xp.isclose(x,y,rtol=1e-10) else 0.0
    ...
    def par2fun(self, p):
        return self.mean + ((self.eigvec@xp.diag(xp.sqrt(self.eigval))) @ p)
    ...
    def _compute_eigpairs(self, xnod, C_nu, sigma, M, N_GL):
        ...
        L, h = xp.linalg.eig(A)                         # solve eigenvalue problem
        # L, h = sp.sparse.linalg.eigsh(A, M, which='LM')   # xp.linalg.eig(A)         
        idx = xp.argsort(-xp.real(L))                  # index sorting descending
        
        # order the results
        eigval = xp.real(L[idx])
        h = h[:,idx]
...
class StepExpansion(Continuous1D):
    ...
    def __init__(self, grid, n_steps=3, fun2par_projection='mean', **kwargs):
        ...
        for i in range(self._n_steps):
            ...
            if i ==0:
                interval_indices, =  xp.where((self.grid>=start)&(self.grid<=end))
            else:
                interval_indices, = xp.where((self.grid>start)&(self.grid<=end))

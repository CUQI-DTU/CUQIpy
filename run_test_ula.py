...
# Set up old ULA sampler
np.random.seed(0)
sampler_old = cuqi.sampler.ULA(target, scale=eps**2, x0=x0)
# Sample
samples_old = sampler_old.sample(N)
plt.figure()
samples_old.plot_chain()
plt.title('old uLA')
# Set up new ULA sampler
np.random.seed(0)
sampler = cuqi.mcmc.ULA_new(target, scale=eps**2, initial_point=x0)
# Sample
sampler.sample(N)
...
# %% test checkpointing with the new ULA sampler
np.random.seed(0)
...
sampler2 = cuqi.mcmc.ULA_new(target, scale=eps**2, initial_point=x0)

...
# define model 
blur_size = 48
kernel = lambda x, y: blur_size / 2*np.exp(-blur_size*abs((x-y)))   # blurring kernel
model = cuqi.model.Deconv_1D(N=N, L=L, kernel=kernel, field_type=None, KL_map=KL_map)
...
d_KL = 100
model = cuqi.model.Deconv_1D(N=N, L=L, kernel=kernel, field_type='CustomKL', \
...
model = cuqi.model.Deconv_1D(N=N, L=L, kernel=kernel, field_type='Step', KL_map=KL_map)
x = model.domain_geometry.grid
#

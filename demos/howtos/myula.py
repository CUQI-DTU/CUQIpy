...
# To sample from $\pi(x|y)$, we are going to apply a ULA based algorithm, MYULA.
# We recall that ULA  
...
# with $prox_g^\alpha (x) = \operatorname{argmin}_z \|x-z \|_2^2/2\alpha + g(z) $.
...
# $\alpha$ corresponds to the smoothing strength. 
...
# .. math::
#    \begin{align*}
#    \mathbf{x} &\sim \exp (-\gamma \|Lx \|_{1,1}\\
...
# As suggested by Durmus et al., we set the smoothing parameter $\alpha =\sigma^2$, ie $\texttt{strength_smooth}=sigma2$.
# We set the regularization parameter to $\texttt{stength_reg}=10$.
from skimage.restoration import denoise_tv_chambolle
strength_reg = 10
strength_smooth = np.copy(sigma2)
...
# %%
# In this section we define the important parameters for MYULA. We let run MYULA for $\texttt{Ns}=10^5$
# iterations. We discard the $\texttt{Nb}=5000$ first samples of the Markov chain, as they are not
# sampled from the stationary distribution of the Markov chain. Furthermore, as MCMC methods generate
# correlated samples, we also perform a thinning, ie we only consider 1 samples every $\texttt{Nt}=100$
# samples to compute our quantities of interest.
# The scale parameter $\delta$ is set wrt the recommendation of Durmus et al.
Ns = 100000
Nb = 5000
Nt = 100
# Step-size of MYULA

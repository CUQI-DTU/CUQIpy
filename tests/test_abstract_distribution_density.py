...
def test_conditioning_variables(rv, expected):
    assert rv.get_conditioning_variables() == expected
    assert rv.dist.get_conditioning_variables() == expected
...
def test_conditioning_through_likelihood(): #TODO. Add more dists to test here!
    ...
    rv = cuqi.distribution.Gaussian(mean=model)
    likelihood = rv.dist.to_likelihood(5)
...
def test_conditioning_wrong_keyword():
    ...
    with pytest.raises(ValueError):
        x.condition(name="hey") #Should expect value-error since name not conditioning variable.
...
def test_conditioning_on_main_parameter():
    ...
    assert isinstance(x.condition(mean=1, x=5).dist, cuqi.likelihood.Likelihood)
    assert isinstance(x.condition(mean=1, cov=1, x=5).dist, cuqi.density.EvaluatedDensity)
    
    # Now using positional arguments
    assert isinstance(x.condition(1, 1, 5).dist, cuqi.density.EvaluatedDensity)
...
def test_conditioning_kwarg_as_mutable_var():
    ...
    x = cuqi.distribution.Gaussian(mean=1, cov=1)
    x = x.condition(cov=2) #This should be ok and not throw an error
    assert x.dist.cov == 2
...
def test_conditioning_both_args_kwargs():
    ...
    x = cuqi.distribution.Gaussian(mean=1)
    with pytest.raises(ValueError):
        x.condition(1, cov=1) #Should expect value-error since cov is given as arg and kwarg.
...
def test_conditioning_multiple_args():
    ...
    rv = cuqi.distribution.Gaussian(mean=lambda x,y: x+y, cov=lambda y,z: y+z, geometry=1)
    assert rv.get_conditioning_variables() == ["x","y","z"]

def test_conditioning_partial_function():
    """ This tests the partial functions we define if only part of a callable is given. """
    rv = cuqi.distribution.Gaussian(mean=lambda x,y: x+y, cov=lambda y,z: y+z, geometry=1)
    rv2 = rv.condition(x=1, y=2)
    assert rv2.get_conditioning_variables() == ["z"]
    assert rv2.dist.mean == 3
    assert cuqi.utilities.get_non_default_args(rv2.dist.cov) == ["z"]
...
def test_conditioning_keeps_name():
    ...
    assert y.condition(x=1, cov=1).name == y.name
    assert y.condition(x=1).condition(cov=1).name == y.name
    assert y.condition(x=1).condition(cov=1).condition().name == y.name

    assert y.condition(x=1).name == y.name
    assert y.condition(cov=1).name == y.name
    assert y.condition().name == y.name
...
def test_conditioning_class_flow():
    ...
    y = cuqi.distribution.Gaussian(lambda x:x, geometry=1)
    Y = y.dist # Extract distribution class
...
def test_logp_conditional():
    ...
    X = x.dist # Extract distribution class
...
def test_logd_positional_and_kwargs():
    ...
    logd = x.condition(mean=3, s=7).logd(13)

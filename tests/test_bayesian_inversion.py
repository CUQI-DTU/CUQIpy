...
@pytest.mark.parametrize("not_legacy", [False, True])
def test_TP_BayesianProblem_sample(copy_reference, TP_type, phantom, prior, Ns, use_legacy, not_legacy):
    ...
    # Sample posterior
    samples = TP.sample_posterior(Ns=Ns, not_legacy=not_legacy)
...
def test_Bayesian_inversion_hierarchical(TP_type: BayesianProblem, phantom: str, priors: Dict[str, Density], Ns: int, not_legacy: bool):
    ...
    if len(priors) == 1: # No hyperparameters
        samples = BP.UQ(Ns=Ns, exact=probInfo.exactSolution, not_legacy=not_legacy)
    else:
        samples = BP.UQ(Ns=Ns, exact={priors[0].name: probInfo.exactSolution}, not_legacy=not_legacy)
